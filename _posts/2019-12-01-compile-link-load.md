...
如图，北桥都是连接的高速设备，cpu、内存、显卡等，采用PCI总线串在一起；南桥慢，都是鼠标、键盘、硬盘等，采用ISA总线串在一起。

CPU主频据说到了4GHz就到头了，所以，就开始多核了，我看看我笔记本上，服务器上，都是2.5-3GHz的，现在卖的多核，其实就是处理器多个，内部缓存共享打包在一起。

硬盘硬件上是盘片、磁道、扇区这样的一个结构，太复杂了，所以，凑头到位给这些扇区编个号，就是所谓的“LBA（Logical Block Address）”的一个逻辑扇区的概念，方便寻址。
...
命名上，x86一般都是指32位；x86-64一般都是指64位。

32位寄存器，一般都是e开头，64位寄存器约定上都是以r开头。
...
参考：<https://nifengz.com/introduction_x64_assembly/>
